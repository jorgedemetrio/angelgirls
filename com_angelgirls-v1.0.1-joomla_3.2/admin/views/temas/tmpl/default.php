...
?>
<form action="<?php echo JRoute::_('index.php?option=com_angelgirls&view=temas'); ?>" method="post" name="adminForm" id="adminForm">
...
<form action="index.php" method="post" name="adminForm" id="adminForm">
        ...
        <?php
			...
			if($lista!=null){
				foreach($lista as $row){
					...
					$link = JRoute::_('index.php?option=com_angelgirls&view=temas&task=editTema&id='. $row->id, false);
					$imagem = JURI::base( true ) . '/../images/temas/' .( isset($row->nome_foto) && $row->nome_foto!=null && $row->nome_foto!=""  ? $row->nome_foto : 'no_image.png');
			?>
    ...
    <input type="hidden" name="task" value="" />
    <input type="hidden" name="boxchecked" value="0" />
    <input type="hidden" name="view" value="temas" />
    <input type="hidden" name="controller" value="temas" />
</form>
