...
class AngelgirlsController extends JControllerLegacy
{
	...
	public function editTema(){
		...
		$query->select($db->quoteName(array('a.id','a.nome','a.descricao','a.meta_descricao','a.audiencia_gostou','a.audiencia_ngostou','a.audiencia_view',
				'a.status_dado','a.id_usuario_criador','a.id_usuario_alterador','a.data_criado','a.data_alterado',
				'b.name','c.name'),
				array('id','nome','descricao','meta_descricao','audiencia_gostou','audiencia_ngostou','audiencia_view',
				...
				->where(' and ' . $db->quoteName('status_dado') . ' <> \'REMOVED\' ');
	...
	public function saveTemaDB(){
		...
		if(isset( $foto)){
			...
			if(!JFile::upload($fileTemp, $uploadPath.$id.'.'.$uploadedFileExtension)){
	...
	public function saveAgendaDB(){
		...
		if($id!=null && $id!=0){//UPDATE
			...
			JRequest::setInt('id',$db->insertid());
		}
