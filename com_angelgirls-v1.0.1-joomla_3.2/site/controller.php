 ...
 *										UTILS 
 *****************************************************************************************************************************************************************/
	private function removerAcentos($valor){
		return preg_replace( '/[`^~\'"]/', null, iconv( 'UTF-8', 'ASCII//TRANSLIT', $valor ) );
	} 

	private function getAlias($valor){
		return strtolower(str_replace(" ","-",removerAcentos( $valor) ) );
	}
...
/****************************************************************************************************************************************************************/
	...
	function sitemapModelos(){
		...
		$xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<urlset xmlns=\"http://www.sitemaps.org/schemas/sitemap/0.9\">\n";
		foreach ( $results as $result){
			$url = $_SERVER['HTTP_HOST'] . JRoute::_('index.php?option=com_angelgirls&view=modelo&task=modelo&id='.$result->id.':modelo-'.$this->getAlias($result->alias),false);
			$xml = $xml . "\t<url>\n";
			$xml = $xml . "\t\t<lastmod>" . JFactory::getDate($result->modified)->format('Y-m-d\TH:i:sP')  . "</lastmod>\n";
			$xml = $xml . "\t\t<changefreq>monthly</changefreq>\n";
	...
	public function sitemapFotografos(){
		...
		$xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<urlset xmlns=\"http://www.sitemaps.org/schemas/sitemap/0.9\">\n";
		foreach ( $results as $result){
			$url = $_SERVER['HTTP_HOST'] . JRoute::_('index.php?option=com_angelgirls&view=fotografo&task=fotografo&id='.$result->id.':fotografo-'.$this->getAlias($result->alias),false);
			$xml = $xml . "\t<url>\n";
			$xml = $xml . "\t\t<lastmod>" . JFactory::getDate($result->modified)->format('Y-m-d\TH:i:sP')  . "</lastmod>\n";
			$xml = $xml . "\t\t<changefreq>monthly</changefreq>\n";
			$xml = $xml . "\t\t<priority>0.2</priority>\n";
	...
	function sitemapSessao(){
		...
		$xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<urlset xmlns=\"http://www.sitemaps.org/schemas/sitemap/0.9\">\n";
		foreach ( $results as $result){
			$url = $_SERVER['HTTP_HOST'] . JRoute::_('index.php?option=com_angelgirls&view=sessoes&task=carregarSessao&id='.$result->id.':sessao-fotografica-'.$this->getAlias($result->alias),false);
			$xml = $xml . "\t<url>\n";
			$xml = $xml . "\t\t<lastmod>" . JFactory::getDate($result->modified)->format('Y-m-d\TH:i:sP')  . "</lastmod>\n";
			$xml = $xml . "\t\t<changefreq>monthly</changefreq>\n";
	...
	function sitemapFotos(){
		...
		$xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<urlset xmlns=\"http://www.sitemaps.org/schemas/sitemap/0.9\">\n";
		foreach ( $results as $result){
			$url = $_SERVER['HTTP_HOST'] . JRoute::_('index.php?option=com_angelgirls&view=sessoes&task=carregarFoto&id='.$result->id.':foto-sensual-'.$this->getAlias($result->alias),false);
			$xml = $xml . "\t<url>\n";
			$xml = $xml . "\t\t<lastmod>" . JFactory::getDate($result->modified)->format('Y-m-d\TH:i:sP')  . "</lastmod>\n";
			$xml = $xml . "\t\t<changefreq>monthly</changefreq>\n";
			$xml = $xml . "\t\t<priority>0.9</priority>\n";
	...
	function sitemapAlbuns(){
		...
		$xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<urlset xmlns=\"http://www.sitemaps.org/schemas/sitemap/0.9\">\n";
		foreach ( $results as $result){
			$url = $_SERVER['HTTP_HOST'] . JRoute::_('index.php?option=com_angelgirls&view=albuns&task=carregarAlbum&id='.$result->id.':album-'.$this->getAlias($result->alias),false);
			$xml = $xml . "\t<url>\n";
			$xml = $xml . "\t\t<lastmod>" . JFactory::getDate($result->modified)->format('Y-m-d\TH:i:sP')  . "</lastmod>\n";
			$xml = $xml . "\t\t<changefreq>monthly</changefreq>\n";
			$xml = $xml . "\t\t<priority>0.9</priority>\n";
	...
	function sitemapAlbumFotos(){
		...
		$xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<urlset xmlns=\"http://www.sitemaps.org/schemas/sitemap/0.9\">\n";
		foreach ( $results as $result){
			$url = $_SERVER['HTTP_HOST'] . JRoute::_('index.php?option=com_angelgirls&view=albuns&task=carregarFotoAlbum&id='.$result->id.':foto-album-'.$this->getAlias($result->alias),false);
			$xml = $xml . "\t<url>\n";
			$xml = $xml . "\t\t<lastmod>" . JFactory::getDate($result->modified)->format('Y-m-d\TH:i:sP')  . "</lastmod>\n";
			$xml = $xml . "\t\t<changefreq>monthly</changefreq>\n";
			$xml = $xml . "\t\t<priority>0.9</priority>\n";
	...
	/**
	 * Valida se o usuário já está cadastrado.
	 */
 ...
 */	
	...
	private function getPerfilFotografoByToken($id){
		$user = JFactory::getUser();
		$db = JFactory::getDbo ();
		$query = $db->getQuery ( true );
	...
	private function getPerfilByToken($token, $tipo='VISITANTE'){
		$db = JFactory::getDbo();
		$user = JFactory::getUser();
		$query = $db->getQuery ( true );
		$query->select('`p`.`id`,`p`.`tipo`,`p`.`usuario`,`p`.`nome_completo`,`p`.`email_principal`,`p`.`id_usuario`,`p`.`apelido`,`p`.`descricao`,`p`.`meta_descricao`,`p`.`foto_perfil`,`p`.`pontos`,`p`.`nivel`,
						...
						`p`.`cidade_nasceu`,`p`.`uf` as `estado_mora`, `p`.`cidade_mora`,
						CASE isnull(`vt_f`.`data_criado` ) WHEN 1 THEN \'NAO\' ELSE \'SIM\' END AS `gostei`')
					->from ( $db->quoteName ( '#__angelgirls_perfil', 'p' ) )
					->join ( 'LEFT', '#__users AS u ON ' . $db->quoteName ( 'p.id_usuario' ) . ' = ' . $db->quoteName('u.id'))
					->join ( 'LEFT', '(SELECT data_criado, id_'.(strtolower($tipo)).' FROM #__angelgirls_vt_'.(strtolower($tipo)).' WHERE id_usuario='.$user->id.') vt_f ON ' . $db->quoteName ( 'p.id' ) . ' = ' . $db->quoteName('vt_f.id_'.(strtolower($tipo)).''))
					->where ( $db->quoteName ( 'p.status_dado' ) . ' NOT IN (' . $db->quote(StatusDado::REMOVIDO) . ') ' )
					->where ( $db->quoteName ( 'p.token' ) . ' =  ' . $db->quote($token) );
		if(isset($tipo)){
			$query->where ( $db->quoteName ( 'p.tipo' ) . ' =  ' . $db->quote(strtoupper($tipo)) );
		}
		$db->setQuery ( $query );
	...
	private function getPerfilModeloByToken($id){
		$db = JFactory::getDbo();
		$user = JFactory::getUser();
		$query = $db->getQuery ( true );
		$query->select('`f`.`id`, `f`.`id_usuario`, `f`.`nome_artistico` AS `nome`,`f`.`audiencia_gostou`, `f`.`meta_descricao`, `f`.`descricao`, `f`.`token`,
	...
	public function perfil(){
		...
		$tipo = JRequest::getString('tipo', JRequest::getString('view',null));
		...
		$usuario = $this->getPerfilByToken($id,$tipo);
		if(!isset($usuario)){
			$this->RegistroNaoEncontado();
			exit();
		}
		JRequest::setVar ( 'perfil', $usuario );
		$tipo = $usuario->tipo;
		$query = $db->getQuery ( true );
		$query->update($db->quoteName('#__angelgirls_'.strtolower($tipo) ))
		->set(array($db->quoteName ( 'audiencia_view' ) . ' = (' . $db->quoteName ( 'audiencia_view' ) .' + 1) '))
		->where ($db->quoteName ( 'id' ) . ' = ' . $db->quote($usuario->id));
		$db->setQuery ( $query );
		$db->execute ();
		
		//Dados de sessão
		if($tipo=='MODELO' || $tipo='FOTOGRAFO'){
			//Tema preferido
			$query = $db->getQuery ( true );
			...
			if($tipo=='MODELO'){
				$query->where (' ( ' . $db->quoteName ('s.id_modelo_principal') . ' = ' . $usuario->id . ' OR ' . $db->quoteName ('s.id_modelo_secundaria') . ' = ' . $usuario->id . ' ) ');
			}
			else{
				$query->where (' ( ' . $db->quoteName ('s.id_fotografo_principal') . ' = ' . $usuario->id . ' OR ' . $db->quoteName ('s.id_fotografo_secundario') . ' = ' . $usuario->id . ' ) ');
			}
			$query->order('total DESC,`f`.`nome`')
			->group(' `f`.`id`, `f`.`nome`')
			->setLimit(1);
			$db->setQuery ( $query );
			...
			$query = $db->getQuery ( true );
			if($tipo=='MODELO'){
				$query->select('`f`.`id`, `f`.`nome_artistico` AS `nome`, count(1) as total, `f`.`token`')
				->from ( $db->quoteName ( '#__angelgirls_fotografo', 'f' ) )
				->join ('INNER',$db->quoteName ('#__angelgirls_sessao', 's' ) . ' ON s.id_fotografo_principal = f.id OR s.id_fotografo_secundario = f.id')
				->where ( $db->quoteName ( 'f.status_dado' ) . ' IN (' . $db->quote(StatusDado::ATIVO) . ',' . $db->quote(StatusDado::NOVO) . ') ' )
				->where ( $db->quoteName ( 's.status_dado' ) . ' IN (' . $db->quote(StatusDado::PUBLICADO) . ') ' )
				->where ( $db->quoteName ( 's.publicar' ) . " <= NOW() " )
				->where (' ( ' . $db->quoteName ('s.id_modelo_principal') . ' = ' . $usuario->id . ' OR ' . $db->quoteName ('s.id_modelo_secundaria') . ' = ' . $usuario->id . ' ) ');
			}
			else{
				$query->select('`f`.`id`, `f`.`nome_artistico` AS `nome`, count(1) as total, `f`.`token`')
				->from ( $db->quoteName ( '#__angelgirls_modelo', 'f' ) )
				...
				->where ( $db->quoteName ( 's.status_dado' ) . ' IN (' . $db->quote(StatusDado::PUBLICADO) . ') ' )
				->where ( $db->quoteName ( 's.publicar' ) . " <= NOW() " )
				->where (' ( ' . $db->quoteName ('s.id_fotografo_principal') . ' = ' . $usuario->id . ' OR ' . $db->quoteName ('s.id_fotografo_secundario') . ' = ' . $usuario->id . ' ) ');
			}
			$query->order('total DESC,`f`.`nome_artistico`')
			->group(' `f`.`id`, `f`.`token`, `f`.`nome_artistico`')
			->setLimit(3);
			$db->setQuery ( $query );
			...
			$query->order('s.audiencia_gostou DESC,s.audiencia_view DESC, `s`.`publicar` DESC ');
			$query->setLimit(6);
			$db->setQuery ( $query );
			$results = $db->loadObjectList();
			...
			$query->order('s.audiencia_view DESC, s.audiencia_gostou DESC, `s`.`publicar` DESC ');
			$query->setLimit(6);
			$db->setQuery ( $query );
			$results = $db->loadObjectList();
 ...
 */	
	...
	public function reprovarSessao(){
		...
		require_once 'views/sessoes/tmpl/repovar_from.php';
		echo("<script>window.location='".JRoute::_('index.php?option=com_angelgirls&view=sessoes&task=carregarMinhasSessoes',false) ."'; \\parent.document.AngelGirls.FrameModalHide();</script>");
	}
	...
	public function removerFotoSessaoJson(){
		...
		if(isset($id) && $id!=0){
			try {
				...
				if(isset($result)){
					$this->MoverImagemParaPastaLixo($result->token_imagem, PATH_IMAGEM_SESSOES . $result->token);
				}
				$query = $db->getQuery ( true );
				$query->update($db->quoteName('#__angelgirls_foto_sessao' ))
				->set(array (
		...
		if (isset($imagem) && JFile::exists($imagem ['tmp_name'])) {
			...
			if(isset($result) && isset($result->token) && strlen(trim($result->token))>=1){
				...
				->columns(array(
						...
						$url = JRoute::_('index.php?option=com_angelgirls&view=sessoes&task=carregarFoto&id='.$foto->id.':foto-sensual-'.$this->getAlias($foto->titulo));
 ...
 */
	...
	private function MoverImagemParaPastaLixo($ArquivoAntigoApagavel, $dest){
		$user = JFactory::getUser();
	...
	private function SalvarUploadImagem($upload, $dest, $arquivo, $tabela=null, $campo=null, $id = 0, $gerarImagens=true, $ComLogo=true, $ArquivoAntigoApagavel=null){
		...
		if(isset($upload) && JFile::exists ( $upload ['tmp_name'] )){
			...
			if(isset($ArquivoAntigoApagavel) && JFile::exists ( $dest . DS . $ArquivoAntigoApagavel)){
				$this->MoverImagemParaPastaLixo($ArquivoAntigoApagavel, $dest);
			}
...
// 				}
			...
			$detalhe = explode ( ' ', $tipo );
			$query = $db->getQuery ( true );
			if(isset($detalhe[1]) && $detalhe[1]=='horizontal'){
			...
			$result = $db->loadObject();
			if(isset($result)){
				$arquivo =  PATH_IMAGEM_MODELOS . (trim(strtolower($tipo)) != 'full'? trim(strtolower($tipo)) . '_':'') . $result->foto;
			...
			$result = $db->loadObject();
			if(isset($result)){
				$arquivo =  PATH_IMAGEM_FOTOGRAFOS . (trim(strtolower($tipo)) != 'full'? trim(strtolower($tipo)) . '_':'') . $result->foto;
				$nomeArquivo = $result->nome;
			}
		}
		//echo($arquivo);exit();
...
/***********************************************************************************************************************/
	public function carregarFotoAlbum(){
		...
		$query = $db->getQuery ( true );
		$query->select('`f`.`id`,`f`.`titulo`,`f`.`descricao`,`f`.`meta_descricao`,`f`.`id_album`,`f`.`audiencia_gostou`,`f`.`token`,
						`s`.`id` AS `id_album`,`s`.`titulo` AS `titulo_album`,`s`.`nome_foto`,
	...
	public function reprovarAlbum(){
		...
		$todosAprovaram = true;
		if($album->status_modelo_principal != 1 || $album->status_fotografo_principal != 1){
			$todosAprovaram = false;
		}
		if(isset($album->id_fotografo_secundario) && $album->id_fotografo_secundario>0
				&& $album->status_fotografo_secundario != 1){
			$todosAprovaram = false;
		}
		if(isset($album->id_modelo_secundaria) && $album->id_modelo_secundaria>0
				&& $album->status_modelo_secundaria != 1){
			$todosAprovaram = false;
		}
		//Montando o texto da mensgem.
		$titulo = 'Sess&atilde;o reprovada pel' . ($perfil->sexo=='M'?'o':'a') . ' ' . $perfil->nome;
		$texto = ' sua sess&atilde;o "' . $album->titulo . '" acaba de ser reprovada por um integrante.<br> ';
		$texto = $texto . 'O motivo foi: <br/>'.$descricao;
		$criador = $this->getPerfilById($album->id_usuario_criador);
		//Alterando status
		if($perfil->id == $album->id_fotografo_principal && $perfil->tipo == 'FOTOGRAFO'){
			$query = $db->getQuery ( true );
		...
		if($perfil->id == $album->id_fotografo_secundario && $perfil->tipo=='FOTOGRAFO'){
			$query = $db->getQuery ( true );
			$query->update($db->quoteName ('#__angelgirls_album'))
			->set (array(
					$db->quoteName ( 'status_dado' ) . ' = ' . $db->quoteName(StatusDado::NOVO) ,
		...
		if($perfil->id == $album->id_modelo_principal && $perfil->tipo=='MODELO'){
			$query = $db->getQuery ( true );
			$query->update($db->quoteName ('#__angelgirls_album'))
			->set (array(
					$db->quoteName ( 'status_dado' ) . ' = ' . $db->quote(StatusDado::NOVO) ,
		...
		if($perfil->id == $album->id_modelo_secundaria && $perfil->tipo=='MODELO'){
			$query = $db->getQuery ( true );
			$query->update($db->quoteName ('#__angelgirls_album'))
			->set (array(
					$db->quoteName ( 'status_dado' ) . ' = ' . $db->quoteName(StatusDado::NOVO) ,
		...
		try{
			//	Enviando mensagem
			$this->EnviarMensagemEmail($criador->email, $criador->nome, TipoMensagens::SESSAO_REJEICAO_EQUIPE, $titulo, $texto);
			$this->EnviarMensagemInbox($titulo, $album->id_usuario_criador, $texto, TipoMensagens::SESSAO_REJEICAO_EQUIPE);
		}
		catch(Exception $e){
			JLog::add($e->getMessage(), JLog::WARNING);
		}
		require_once 'views/albuns/tmpl/repovar_from.php';
		echo("<script>window.location='".JRoute::_('index.php?option=com_angelgirls&view=albuns&task=carregarMeusAlbuns',false) ."'; \\parent.document.AngelGirls.FrameModalHide();</script>");
	}
	 ...
	 * 
	 */
	public function carregarEditarAlbum(){
		$id = JRequest::getString('id',0);
		if(!(strpos($id,':')===false)){
			$var =explode(':',$id); 
			$id = $var[0];
		...
		JRequest::setVar ( 'fotos', $this->runFotoAlbum($user, 0, $id, $this::LIMIT_DEFAULT) );
		...
		$query->select('count(s.id) AS total ')
					...
					->where ( $db->quoteName ( 's.status_dado' ) . ' NOT IN (' . $db->quote(StatusDado::REMOVIDO) . ') ' )
					->where ( $db->quoteName ( 's.id_album' ) . " =  " . $id)
					->where ( $db->quoteName ( 's.possui_nudes' ) . " =  'N' ");
		$db->setQuery ( $query );
		$result = $db->loadObject();
		JRequest::setVar('sem_nudes', $result->total);
	...
	public function salvarAlbum(){
		...
		$historia = JRequest::getString('historia',null,'POST');
		$locacao  = JRequest::getInt('locacao',null,'POST');
		$tipo_album = JRequest::getString('tipo_album',null,'POST');
		$id_figurino_principal  = JRequest::getInt('id_figurino_principal',null,'POST');
		$id_figurino_secundario  = JRequest::getInt('id_figurino_secundario',null,'POST');
		$id_modelo_principal  = JRequest::getInt('id_modelo_principal',null,'POST');
		$id_modelo_secundaria  = JRequest::getInt('id_modelo_secundaria',null,'POST');
		...
		if(isset($data_realizada) && strlen($data_realizada) >= 8){
			$dataRealizadoAlbum = DateTime::createFromFormat('d/m/Y H:i:s', $data_realizada.' 00:00:00');
			
			if(intval($dataRealizadoAlbum->format('Ymd')) >= intval(date('Ymd'))){
				JError::raiseWarning(100,JText::_('O campo "Data da realiza&ccedil;&atilde;o da sess&atilde;o" deve ser inferior a hoje. E deve estar no formato DD/MM/AAAA'));
				$erros = true;
			}
			
			$dataFormatadaBanco = $db->quote($dataRealizadoAlbum->format('Y-m-d'));
		}
		else{
			JError::raiseWarning(100,JText::_('O campo "Data da realiza&ccedil;&atilde;o da sess&atilde;o" &eacute; um campo obrigat&oacute;rio. E deve estar no formato DD/MM/AAAA'));
			$erros = true;
		...
		if(!isset($tipo_album) || strlen(trim($tipo_album)) <=0){
			JError::raiseWarning(100,JText::_('O campo "Tipo" &eacute; um campo obrigat&oacute;rio.'));
			$erros = true;
		}
		...
		if($erros){
			$this->carregarEditarAlbum();
			return;
		}
		if(!isset($id) || $id==0 || strlen(trim($id)) <= 0 ){
			$query = $db->getQuery ( true );
			$query->insert( $db->quoteName ( '#__angelgirls_album' ))
			->columns ( array (
				$db->quoteName ( 'status_dado' ),
				$db->quoteName ( 'data_criado' ),
				$db->quoteName ( 'id_usuario_criador' ),
			...
			->values ( implode ( ',', array (
					...
					($perfil->tipo=='MODELO'?'1':'null'),
					($perfil->tipo=='FOTOGRAFO'?'1':'null'),
					(!isset($tipo_album) || strlen(trim($tipo_album))<=0  ? ' null ' :  $db->quote($tipo_album)),
					$db->quote($this->getRemoteHostIp()),
					$db->quote($this->getRemoteHostIp())
			)));
			$db->setQuery( $query );
			...
			if (isset ( $imagem ) && JFile::exists ( $imagem ['tmp_name'] )) {
				...
				$this->SalvarUploadImagem($imagem,
						PATH_IMAGEM_SESSOES . $token,
						$this->GerarNovoNomeArquivo($imagem['name'], $id ),
						'#__angelgirls_album','nome_foto',$id,true,true);
			}
		}
		else{
			$query = $db->getQuery ( true );
			...
			if(isset($result)){
				...
				$query->update( $db->quoteName ( '#__angelgirls_album' ))
					  ->set (array(
					  		...
					  		$db->quoteName ( 'id_figurino_principal' ) . ' = ' . (!isset($id_figurino_principal) || strlen(trim($id_figurino_principal))<=0 || $id_figurino_principal==0 ? ' null ' : $id_figurino_principal),
					  		$db->quoteName ( 'id_figurino_secundario' ) . ' = ' . (!isset($id_figurino_secundario) || strlen(trim($id_figurino_secundario))<=0 || $id_figurino_secundario==0 ? ' null ' : $id_figurino_secundario),
					  		$db->quoteName ( 'tipo' ) . ' = ' . (!isset($tipo_album) || strlen(trim($tipo_album))<=0  ? ' null ' : $db->quote($tipo_album)),
							$db->quoteName ( 'host_ip_alterador' ) . ' = ' . $db->quote($this->getRemoteHostIp())
					  ))
					->where ( $db->quoteName ( 'id_usuario_criador' ) . " =  " . $user->id )
					->where ( $db->quoteName ( 'id' ) . " =  " . $id );
				...
				if (isset ( $imagem ) && JFile::exists ( $imagem ['tmp_name'] )) {
					$this->SalvarUploadImagem($imagem,
							PATH_IMAGEM_SESSOES . $token,
							$this->GerarNovoNomeArquivo($imagem['name'], $id ),
							'#__angelgirls_album','nome_foto',$id,true,true, $NomeArquivoAntigo);
				}
			}
			else{
				JError::raiseWarning( 100, 'O informa&ccedi;&atilde;o que tentou salvar n&atilde;o exite.' );
		...
		if($publicar=='S'){
			if($this->publicarAlbum()){
				JFactory::getApplication()->enqueueMessage(JText::_('Sess&atilde;o publicada com sucesso!<br/>Aguardando aprova&ccedil;&atilde;o da(s) modelo(s) e fotografo(s) para ir para analize pelo comite.'));
				$this->carregarMeusAlbuns();
			}
			else{
				$this->carregarEditarAlbum();
			}
	...
	public function salvarAlteracaoFotoAlbum(){
		$user = JFactory::getUser();
		$db = JFactory::getDbo ();
		...
		->join('INNER','#__angelgirls_foto_album as f ON s.id = f.id_album')
		->where ( $db->quoteName ( 'f.id' ) . " =  " . $id );
		$db->setQuery ( $query );
		$album = $db->loadObject();
		...
		if($aplicarTodos!='S'){
			...
			$query->where ($db->quoteName ( 'id_album' ) . ' = ' . $album->id_album);
		}
		...
		require_once 'views/albuns/tmpl/editar_foto.php';
		if($aplicarTodos=='S'){
			echo("<script>jQuery('.labelsFotos',parent.document).html('".$titulo."');");
		}
		else{
	 ...
	 *
	 */
	public function alterarPossuiNudesFotoAlbumJSon(){
		$user = JFactory::getUser();
		$db = JFactory::getDbo ();
		$id =  JRequest::getInt('id');
	...
	public function carregarEditarFotoAlbum(){
		$user = JFactory::getUser();
		$db = JFactory::getDbo ();
		$id =  JRequest::getVar('id');
	
		$query = $db->getQuery ( true );
		$query->select('f.area_vip, f.possui_nudes, f.meta_descricao, f.descricao, f.titulo, s.token as token_album, f.token, f.token_imagem, f.titulo ')
		->from ('#__angelgirls_album as s')
		->join('INNER','#__angelgirls_foto_album as f ON s.id = f.id_album')
		->where ( $db->quoteName ( 's.status_dado' ) . ' NOT IN (' . $db->quote(StatusDado::REMOVIDO) . ',' . $db->quote(StatusDado::PUBLICADO) . ',' . $db->quote(StatusDado::REPROVADO) . ') ' )
		->where ( $db->quoteName ( 'f.status_dado' ) . ' NOT IN (' . $db->quote(StatusDado::REMOVIDO) . ',' . $db->quote(StatusDado::PUBLICADO) . ',' . $db->quote(StatusDado::REPROVADO) . ') ' )
	 ...
	 * Remover Endereco
	 */
	public function removerFotoAlbumJson(){
		$user = JFactory::getUser();
		$db = JFactory::getDbo ();
	...
	public function removerAlbum(){
		...
		->set(array (
				...
				$db->quoteName ( 'data_alterado' ) . ' = NOW()  ',
				$db->quoteName ( 'host_ip_alterador' ) . ' = ' . $db->quote($this->getRemoteHostIp())))
				->where ($db->quoteName ( 'id_album' ) . ' = ' . $id)
				->where ($db->quoteName ( 'id_usuario_criador' ) . ' = ' . $user->id);
		$db->setQuery( $query );
		$db->execute();
	 ...
	 *
	 */
	public function enviarFotosAlbum(){
		$db = JFactory::getDbo();
		...
		if (isset($imagem) && JFile::exists($imagem ['tmp_name'])) {
			...
			if(isset($result) && isset($result->token) && strlen(trim($result->token))>=1){
				...
				$query = $db->getQuery ( true );
				$query->insert( $db->quoteName ( '#__angelgirls_foto_album' ) )
				->columns(array(
						$db->quoteName ( 'status_dado' ),
						$db->quoteName ( 'data_criado' ),
						$db->quoteName ( 'id_usuario_criador' ),
						...
						$url = JRoute::_('index.php?option=com_angelgirls&view=albuns&task=carregarFotoAlbum&id='.$foto->id.':foto-sensual-'.$this->getAlias($foto->titulo));
						$urlIco = JRoute::_('index.php?option=com_angelgirls&view=fotoalbum&task=loadImage&id='.$token.':ico');
						$urlcube = JRoute::_('index.php?option=com_angelgirls&view=fotoalbum&task=loadImage&id='.$token.':cube');
						$urlthumb = JRoute::_('index.php?option=com_angelgirls&view=fotoalbum&task=loadImage&id='.$token.':thumb');
	...
	public function carregarFotosAlbumContinuaHtml(){
		...
		$album = $this->getAlbumById($id);
		JRequest::setVar('album', $album);
		require_once 'views/albuns/tmpl/fotos.php';
		exit();
	}
	
	
	
	private function runFotoAlbum($user, $posicao, $iAlbum, $limit = 0 ){
		$db = JFactory::getDbo ();
	...
	private function runQueryFilterAlbuns($user, $nome, $posicao, $idModelo, $idFotografo, $dataInicio, $dataFim, $ordem, $minha = 'N',$ComLimite =true ){
		$db = JFactory::getDbo ();
		$user = JFactory::getUser();
		$query = $db->getQuery ( true );
		$query->select("`s`.`id` AS `id`,
	...
	public function carregarAlbunsContinuaJson(){
		...
		$results = $this->runQueryFilterAlbuns($user, $nome, $posicao, $idModelo, $idFotografo, $dataInicio, $dataFim, $ordem, $minha );
		JRequest::setVar ( 'albuns', $results );
		require_once 'views/albuns/tmpl/albuns_html.php';
		exit();
	}
	...
	public function carregarMeusAlbuns(){
		$user = JFactory::getUser();
		$nome = JRequest::getString( 'nome', null);
		...
		JRequest::setVar ( 'albuns', $this->runQueryFilterAlbuns($user, $nome, 0, $idModelo, $idFotografo, $dataInicio, $dataFim, $ordem,'T',false));
		...
		JRequest::setVar ( 'view', 'albuns' );
		JRequest::setVar ( 'layout', 'meus_albuns' );
		parent::display ();
	}
...
/*************************************************************************************************************************************/
	...
	public function amigos(){
		...
		$query->select('`p`.`id`,`p`.`tipo`,`p`.`usuario`,`p`.`nome_completo`,`p`.`email_principal`,`p`.`id_usuario`,`p`.`apelido`,`p`.`descricao`,`p`.`meta_descricao`,`p`.`foto_perfil`,`p`.`pontos`,`p`.`nivel`,
						...
						`p`.`cidade_nasceu`,`p`.`uf` as `estado_mora`, `p`.`cidade_mora`')
		->from ( $db->quoteName ( '#__angelgirls_perfil', 'p' ) )
		->join ( 'INNER', '#__angelgirls_amizade s ON ' . $db->quoteName ( 's.id_usuario_solicitado' ) . ' = ' . $db->quoteName('p.id_usuario'))
		->where ( ' p.id_usuario <> ' . $user->id)
		->where ( ' s.id_usuario_solicitante = ' . $user->id)
		->where ( ' s.data_aceita IS NULL ')
		->where ( $db->quoteName ( 'p.status_dado' ) . ' IN (' . $db->quote(StatusDado::ATIVO) . ',' . $db->quote(StatusDado::NOVO) . ') ' )
		->order('p.apelido')
		->limit(1000);
		$db->setQuery ( $query );
		JRequest::setVar('soliticacoes_feitas', $db->loadObjectList());
		...
		$query->select('`p`.`id`,`p`.`tipo`,`p`.`usuario`,`p`.`nome_completo`,`p`.`email_principal`,`p`.`id_usuario`,`p`.`apelido`,`p`.`descricao`,`p`.`meta_descricao`,`p`.`foto_perfil`,`p`.`pontos`,`p`.`nivel`,
						...
						`p`.`cidade_nasceu`,`p`.`uf` as `estado_mora`, `p`.`cidade_mora`')
			->from ( $db->quoteName ( '#__angelgirls_perfil', 'p' ) )
			->join ( 'INNER', '#__angelgirls_amizade s ON ' . $db->quoteName ( 's.id_usuario_solicitante' ) . ' = ' . $db->quoteName('p.id_usuario'))
			->where ( ' p.id_usuario <> ' . $user->id)
			->where ( ' s.id_usuario_solicitado = ' . $user->id)
			->where ( ' s.data_aceita IS NULL ')
			->where ( $db->quoteName ( 'p.status_dado' ) . ' IN (' . $db->quote(StatusDado::ATIVO) . ',' . $db->quote(StatusDado::NOVO) . ') ' )
			->order('p.apelido')
			->limit(1000);
		$db->setQuery ( $query );
		JRequest::setVar('soliticacoes_recebidas', $db->loadObjectList());
		JRequest::setVar('amigos', $this->ListaAmigos());
	...
	public function AmigosHTML(){
		$posicao  = JRequest::getInt('posicao',null);
		
		JRequest::setVar('soliticacoes_recebidas', $db->loadObjectList($this::LIMIT_DEFAULT, $posicao));
	...
	private function ListaAmigos($limite = AngelgirlsController::LIMIT_DEFAULT,$posicao = null){
		$user = JFactory::getUser();
		$db = JFactory::getDbo ();
		$query = $db->getQuery ( true );
		$query->select('`p`.`id`,`p`.`tipo`,`p`.`usuario`,`p`.`nome_completo`,`p`.`email_principal`,`p`.`id_usuario`,`p`.`apelido`,`p`.`descricao`,`p`.`meta_descricao`,`p`.`foto_perfil`,`p`.`pontos`,`p`.`nivel`,
						...
						`p`.`cidade_nasceu`,`p`.`uf` as `estado_mora`, `p`.`cidade_mora`')
			->from ( $db->quoteName ( '#__angelgirls_perfil', 'p' ) )
			->join ( 'INNER', '#__angelgirls_amizade_lista_contato c ON ' . $db->quoteName ( 'c.id_usuario' ) . ' = ' . $db->quoteName('p.id_usuario'))
			->join ( 'INNER', '#__angelgirls_amizade_lista l ON ' . $db->quoteName ( 'l.id' ) . ' = ' . $db->quoteName('c.id_lista'))
			->where ( ' p.id_usuario <> ' . $user->id)
			->where ( ' l.id_usuario_criador = ' . $user->id)
			->where ( $db->quoteName ( 'p.status_dado' ) . ' IN (' . $db->quote(StatusDado::ATIVO) . ',' . $db->quote(StatusDado::NOVO) . ') ' )
			->order('p.apelido');
		if($posicao==null){
			$query->limit($limite);
...
/******************************** REDES SOCIAIS **************************************************/
	...
	public function logado(){
		try{
			...
			if(!isset($perfil->popads) || !$perfil->popads){
			...
			$query = $db->getQuery ( true );
			$query->select("t.id, t.token, t.tipo,  t.titulo, t.descricao, t.prioridade, t.data_publicado, 
							t.audiencia, t.acessos, t.autor1, t.autorid1, t.autor2, t.autorid2, t.rnd, t.opt1, t.opt2, t.opt3, t.opt4,
					CASE t.tipo  
						WHEN 'MODELO'  THEN
...
/***********************************************************************************************************/
	...
	// 		JRequest::setVar ( 'amizade', $db->loadObject() );
			...
			$query = $db->getQuery ( true );
			$query->select(' data_aceita, id_usuario_solicitante ')
			->from ( $db->quoteName ( '#__angelgirls_amizade' ) )
			->where ('((id_usuario_solicitante =  ' . $user->id .' AND id_usuario_solicitado = ' . $id .
					 ') OR (id_usuario_solicitante =  ' . $id .' AND id_usuario_solicitado = ' . $user->id. ') )' );
			$db->setQuery ( $query );
			JRequest::setVar ( 'amizade', $db->loadObject() );
	...
	public function rejeitarSolicitacaoAmizade(){
		...
		try{
			...
			->where ($db->quoteName ( 'id_usuario_solicitante' ).'in  ' . $queryUsuario  )
			...
			$query = $db->getQuery ( true );
			$query->delete( $db->quoteName ('#__angelgirls_amizade_lista_contato') )
			->where ($db->quoteName ( 'id_lista' ).' in (SELECT id from #__angelgirls_amizade_lista WHERE id_usuario_criador in ' . $queryUsuario  . ')'  )
			->where (' id_usuario = ' . $user->id );
			$db->setQuery( $query );
			$db->execute();
		}
	...
	public function cancelarAmizade(){
		...
		try{
			...
			$query = $db->getQuery ( true );
			$query->delete( $db->quoteName ('#__angelgirls_amizade') )
			->where ('( id_usuario_solicitado in  ' . $queryUsuario .' OR  id_usuario_solicitante  in  ' . $queryUsuario . ')')
			->where ('( id_usuario_solicitado =  ' . $user->id .' OR  id_usuario_solicitante  =  ' . $user->id . ')');
			$db->setQuery( $query );
			$db->execute();
	...
	public function solicitarAmizade(){
		...
		try{
			...
			->columns (array (
					$db->quoteName ( 'id_usuario_solicitante' ),
			...
			$url = ''; 
			if($perfil->tipo=='FOTOGRAFO') {
				$url = JRoute::_('index.php?option=com_angelgirls&view=fotografo&task=fotografo&id='.$perfil->token.':fotografo-'.$this->getAlias($perfil->apelido),false);
			}
			else if($perfil->tipo=='MODELO') {
				$url = JRoute::_('index.php?option=com_angelgirls&view=modelo&task=modelo&id='.$perfil->token.':modelo-'.$this->getAlias($perfil->apelido),false);
			}
			else  {
				$url = JRoute::_('index.php?option=com_angelgirls&view=perfil&task=carregarPerfil&id='.$perfil->token.':'.$this->getAlias($perfil->apelido),false);
			}
			
			$mensagem = 'O usu&aacute;rio '.$perfil->nome.' enviou uma solicita&ccedil;&atilde;o de amizade para voc&ecirc;.\n<br/>';
			$mensagem = $mensagem . '<a href="//'. $_SERVER['HTTP_HOST'] . $url .'"> Clique aqui para ver mais </a>';
					
					
			$this->EnviarMensagemInbox('Solicita&ccedil;&atilde;o de amizade', $queryUsuario, $mensagem, TipoMensagens::SOLICITACAO_AMIZADE);
		}
		catch(Exception $e){
			$json = '{"ok":"nok", "mensagem":"'.$e->getMessage().'"}';
		}
	...
	public function aceitarAmizade(){
		...
		try{
			...
			//$perfilSolicitante = $this->getPerfilByToken($token, $tipo);
			...
			->where($db->quoteName ( 'id_usuario_solicitante' ) . ' in ' . $queryUsuario)
			->where($db->quoteName ( 'id_usuario_solicitado' ) . ' = ' . $user->id);
			$db->setQuery ( $query );
			$db->execute();
			$this->LogQuery($query);
			...
			$query->select("`id`")
				...
				->where ('`nome` = \'AMIGOS\'' )
				->where ('`sistema` = \'S\'' )
				->where ('`id_usuario_criador` in  ' . $queryUsuario)
				->setLimit(1);
			$db->setQuery ( $query );
			$listaID = $db->loadObject()->id;
			if(!isset($listaID) || $listaID <= 0){
				$listaID = $this->criarListaDefaultAmigos($queryUsuario);
			}
			...
			->columns (array (
					...
					$db->quoteName ( 'data_alterado' ),
					$db->quoteName ( 'host_ip_criador' )))
					->values(implode(',', array ($listaID, $queryUsuario, 'NOW()', $db->quote($this->getRemoteHostIp()))));
			$db->setQuery( $query );
			if(!$db->execute()){
				$json = '{"ok":"nok", "mensagem":"Falha ao enviar a solicita&ccedil;o!"}';
			}
	...
	private function getContatos($name=null){
		...
		UNION
		(SELECT id_usuario_solicitante AS ID, id_usuario_solicitado AS ID_USER FROM #__angelgirls_amizade)
		UNION
		(SELECT id_usuario_solicitado AS ID, id_usuario_solicitante AS ID_USER FROM #__angelgirls_amizade)
		UNION
	...
	private function criarListaDefaultAmigos($idDonoLista){
		$db = JFactory::getDbo ();
		$query = $db->getQuery ( true );
		...
		->columns (array (
				...
				$db->quoteName ( 'host_ip_criador' ),
				$db->quoteName ( 'host_ip_alterador' )))
				->values(implode(',', array ($db->quote('AMIGOS'), $db->quote('S'), $db->quote(StatusDado::NOVO), 'NOW()', 'NOW()', $idDonoLista, $idDonoLista,
