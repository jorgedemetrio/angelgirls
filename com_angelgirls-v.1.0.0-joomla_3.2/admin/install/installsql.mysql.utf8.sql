...
CREATE TABLE `#__angelgirls_modelo` ( 
		...
		`possui_foto_inteira` ENUM('S','N') NOT NULL , 
		`altura` NUMERIC(5,2) , 
		`peso` NUMERIC(5,2) , 
		`busto` NUMERIC(5,2) , 
		`calsa` NUMERIC(3) , 
		...
		`pele` ENUM('BRANCA','PARDA','MORENA','NEGRA','AMARELA','OUTRO') , 
		`etinia` ENUM('AZIATICA','AFRO','EURPEIA','ORIENTAL','LATINA','OUTRO','OUTRO'), 
...
CREATE TABLE `#__angelgirls_sessao` ( 
		...
		`id_producao_principal` INT  NOT NULL,
		`id_producao_secubdario` INT,

		`id_figurino_principal` INT  NOT NULL,
